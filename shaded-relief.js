...
const resamplingKernel = gpu.createKernel(function(rasterData, outNx, outNy, nx, ny,
    vert,
    dp,
    twoPi,
    halfPi,
    ...
    let xi = nx * this.thread.x/outNx, yi = ny * this.thread.y/outNy;

    var z0 = vert * Math_bicubicInterpolation(rasterData, yi, xi + 1, nx, ny);
    var z1 = vert * Math_bicubicInterpolation(rasterData, yi, xi - 1, nx, ny);
    var dzdx = (z1 - z0) / dp;
    z0 = vert * Math_bicubicInterpolation(rasterData, yi - 1, xi, nx, ny);
    z1 = vert * Math_bicubicInterpolation(rasterData, yi + 1, xi, nx, ny);
    var dzdy = (z1 - z0) / dp;
    ...
    var cosIncidence = sinSunEl * Math.cos(slope) + cosSunEl * Math.sin(slope) * Math.cos(sunAz - aspect);
    ...
    if(shaded>255)
        shaded = 255;
    shaded = 55 + 200 * shaded/255;
    return shaded;
});

const resamplingCpu = (rasterData, outNx, outNy, nx, ny,
    vert,
    dp,
    twoPi,
    halfPi,
    ...
    for(let y=0;y<outNy;y++){
        ...
        for(let x=0;x<outNx;x++){
            let xi = nx * x/outNx, yi = ny * y/outNy;
            var z0 = vert * Math_bicubicInterpolation(rasterData, yi, xi + 1, nx, ny);
            var z1 = vert * Math_bicubicInterpolation(rasterData, yi, xi - 1, nx, ny);
            var dzdx = (z1 - z0) / dp;
            z0 = vert * Math_bicubicInterpolation(rasterData, yi - 1, xi, nx, ny);
...
const start = async () => {
    let data = await RasterUtils.processTiffSource('../data/dem2.tif');
